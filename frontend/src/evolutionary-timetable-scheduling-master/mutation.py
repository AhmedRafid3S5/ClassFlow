...
def neighbour(chromosome):
    ...
    for classroom in chromosome[2]:
        ...
        for k in range(len(chromosome[2][classroom])):
            if chromosome[2][classroom][k] == 0 and k % 6 + length <= 6:
                c += 1
    ...
    if not found:
        ...
        # Friday 8pm is reserved for free hour
        if day == 4:
            period = random.randrange(0, 6 - int(chromosome[0][i]['Length']))
        else:
            period = random.randrange(0, 7 - int(chromosome[0][i]['Length']))
        time = 6 * day + period
...
def neighbour2(chromosome):
    ...
    while not satisfied:
        ...
        if first['Assigned_classroom'] in second['Classroom'] and second['Assigned_classroom'] in first['Classroom']\
                and first['Assigned_time'] % 6 + int(second['Length']) <= 6 \
                and second['Assigned_time'] % 6 + int(first['Length']) <= 6:
            if first['Assigned_time'] + int(second['Length']) != 30 and second['Assigned_time'] + int(first['Length']) != 30\
                    and first != second:
                satisfied = True
        c += 1
    ...
    # Set the classes to new times and places
    for j in range(first['Assigned_time'], first['Assigned_time'] + int(first['Length'])):
        chromosome[1][first['Professor']][j] += 1
        chromosome[2][first['Assigned_classroom']][j] += 1
        for group in first['Group']:
